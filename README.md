...
* **/datasets/data/\<int:id\>?from&?to**
   * [GET /datasets/data/\<int:id\>?from&?to](#get-datasetsdataintidfromto)
* **/run**
   * [GET /run](#get-run)
...
*  `num_rows`: Integer - количество строк в датасете
#### Пример ответа
```json
{
	"id": 1,
...
### GET /datasets/data/\<int:id\>/?from&?to
Принимает id датасета в строке запроса, а также границы интервала в качестве параметров. Возвращает строки датасета в заданном интервале. 
#### Строка запроса
*  `id`: Integer - id датасета
#### Параметры запроса
*  `from`: Integer - левая граница интервала
*  `to`: Integer - правая граница интервала
#### Ответ
*  строки датасета в json формате
#### Пример ответа
```json
